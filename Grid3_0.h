...
private:
	...
	char playerSymbol;
	const char AISymbol = '@';
	const string AIName = "The Computer";
...
double Grid::moveValue(int col){
	...
	double curVal = maxmin(level, numeric_limits<int>::min(), numeric_limits<int>::max(), false);
...
char Grid::getAISymbol(){
	return this->AISymbol;
}

string Grid::getAIName(){
	return this->AIName;
}
...
string Grid::charAt(int row, int col){
